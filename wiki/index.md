...
| [[affiliate-performance]] | **UPDATED 2026-08-09.** July closed **+$92.06** (66% from one order; ex-outlier ~$32). **August 1–4 = $6.84 on 2 orders — naive run rate ~$53/mo, below the $100 gate** (n=2, no weight yet). Gate stays **1 of 2–3**, closes 09-01. **Two export window types exist — check before logging.** Aug 3 and Aug 4 are both rolling-30d and are 93–99% July re-reported. Market benchmark: DataForSEO CPC $3.73/click vs TCA EPC $1.00 ($0.35 ex-outlier) — **monetization at/above category norms, session volume is the constraint.** GA4 undercounts affiliate clicks ~4x. **2026-08-09: automated pulling LIVE (P3).** Daily into `data/affiliate/latest.json` via the nightly, weekly archive into `raw/affiliate/`. Files `amazon-session-expired` rather than ever writing a $0 row. **The daily figure is SHIPPED earnings, not net** — do not copy it into the monthly table. ASIN-level attribution still manual. Open: **5th straight export with 0 chair orders on 92 chair clicks**; Leap Plus clicks flat at 49. |
