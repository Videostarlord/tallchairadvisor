...
| [[affiliate-compliance]] | **NEW 2026-05-11.** FTC body disclosure missing on 6 pages. Revenue leaks: aeron-vs-gesture (0 CTAs in 84%), gesture review (CTA at 85%), best-office-chairs (Quick Picks → internal not Amazon). **2026-08-09: monthly dead-ASIN liveness check added (A10/P4, Firecrawl). Findings are ADVISORY — its first live run produced a false positive that would have removed a working link.** |
| [[affiliate-performance]] | **UPDATED 2026-08-28. ⚠ THE CHAIR TAG CONVERTED — the 08-26 $0.00 finding is REVERSED.** `tcachair-20`: **59 clicks, 6 orders, $943.79, $28.54 earnings ($0.48 EPC, 12.24% product CVR)**, against $0.00 on the same tag two days earlier. **Chair links earn; chair UNITS still never sell** — the 6 items average $157.30 against ~$1,300+ chairs, so it is basket spillover inside the 24h window. **Lesson: $0.00 at low volume is a null result, not a zero rate** — a directive was written from one 12-day reading and had to be revoked in 48 hours. n=6, do not build a plan on this reading either. August close now ~**$35.38** (was ~$12.15 forecast). Window solved again by algebra (Jul 29–Aug 27), method now twice-confirmed. 🚫 **P3 AUTOMATED PULL RETIRED — `amazon-pull.ts` and its workflow are DELETED. Do not rebuild.** A session-replay scraper against a financial account expires every ~11 days and demands a manual login only Jackson may perform; it converted a monthly CSV download into a fortnightly credential chore returning *less* data. Affiliate data is hand-exported, permanently. `data/affiliate/latest.json` is **frozen and kept** — its per-day rows are the **decoder** for hand-dropped CSV windows. **The chair tag returned $0.00.** First export after the 2026-08-13 tracking-ID split: `tcachair-20` = **45 clicks, 0 orders, $0.00** over 12 clean days; with Aug 13's 89 chair clicks that is **134 named chair clicks → $0 across two periods**, now measured directly, not inferred. Decision: **Amazon chair clicks are no longer a revenue lever** ([[decisions-log]]). **TRAP:** `linked-product.csv` books $80.89 (99.7% of earnings) to Leap Plus `B00TYE4QXU` — **not a chair sale**; same items are on the *legacy* tag, dated Jul 27–Aug 4, and $672.69/item cannot be a ~$1,300 chair. **Window SOLVED by algebra** (rolling 30d, Jul 27–Aug 25) against `latest.json`'s daily rows — those rows are the **decoder for every hand export**, a third reason never to overwrite them from a CSV. Totals 131 clicks / **$81.13**, of which $68.98 is old money (legacy tag has earned nothing since **Aug 4**) and **$12.15** is new, all in the `others` bucket at a **100% product CVR** with ~$1,575 unshipped (~$47 at 3%). 🔴 **Automated pull DEAD 17 days** — `AMAZON_STORAGE_STATE` needs re-capture by Jackson before the 09-01 close. 🔴 **August tracking to FAIL the $100 gate at $12.15**; gate stays **1 of 2–3**. New: Crandall Leap V2 `B08PPVCCST` first attributed clicks (11). July closed +$92.06. Market benchmark: DataForSEO CPC $3.73 vs TCA EPC $1.00 — monetization at category norms, **the 3% furniture tier on a $500+ considered purchase is the constraint, not clicks.** |
