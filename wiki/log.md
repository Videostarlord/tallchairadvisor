...
## [2026-07-21] analysis | Post-consolidation GSC check — impressions −27%, clicks +8%, consolidation exonerated

- Jackson flagged falling impressions since the Jul 4 commercial-cluster consolidation. Built a 160-day continuous daily series by merging `dailyTrend` across all `raw/gsc/gsc-*.json` snapshots (Feb 9 → Jul 18) plus page- and pageQuery-level diffs across the Jul 06/13/20 90-day snapshots. Raw: `raw/audits/2026-07-21-post-consolidation-gsc-analysis.md`.
- **Verdict: impressions −27% (1,833 → 1,331/day), clicks +8% (3.10 → 3.36/day), CTR +49% (0.169% → 0.252%).** Week ending Jul 18 tied the best click week in 10 weeks (26) on 57% fewer impressions than four weeks prior. Clicks/day have been flat at ~3.2 since June 1 through the entire impression balloon and deflation.
- **Consolidation is not the cause.** Weekly impression-drop attribution (−5,614 sitewide): `/knee-pain-seat-depth/` −3,285 (58.5%), `/review/leap-plus/` −1,061, `/correct-chair-dimensions/` −640, killed `/best-office-chairs/` **−96 (1.7%)**. The killed page was contributing ~14 impr/day; the drop is ~500/day.
- **Root insight — 97% of site impressions are anonymous and clickless.** Query-attributable impressions flat at ~2,500 per 90d window for two months while totals went 52k → 94k → deflating. `/knee-pain-seat-depth/` is 41% of all impressions at pos 5.7 with 18 clicks (0.047% CTR) and its position *improved* 6.1 → 5.7 while impressions collapsed. Rule recorded in [[gsc-performance]]: track clicks/CTR/revenue, never restructure in response to impression movement.
- **Consolidation mechanics verified clean:** both slash variants 301 single-hop, spec sub-pages 301 to parents, sitemap 43 URLs with no killed URLs, GSC page count 39 → 42 → 43 (no index loss). Survivor stable at pos 8.1, clicks 16 → 17 → 18.
- **Goal not yet delivered:** head-term family still attributed to the dead URL at pos 40.6–68.1, 0 clicks, 17 days post-301. Normal latency 2–8 weeks; hold the Sep 1 evaluation.
- **Success metric revised** (see [[decisions-log]] 2026-W30): "+3 positions" replaced with an absolute threshold. Wiki updated: [[gsc-performance]], [[office-chairs-for-tall-people]], [[knee-pain-seat-depth]], [[best-office-chairs]].

## [2026-07-21] analysis | Profit projections + monetization plan — Gesture branded cluster identified as top lever

- Jackson asked for realistic profit projections and monetization levers. Pulled every source: 27 GSC snapshots, both Amazon Associates reports + 4 CSVs, Clarity latest + 354-row history, GA4, DataForSEO keyword gaps (143 kw), competitor intelligence. Full model: `raw/strategy/2026-07-21-profit-projections-monetization.md`.
- **Unit economics (July 1–17, the only post-ASIN-fix period):** 192 sessions → 82 affiliate clicks (**43% of sessions**) → 6 orders (**7.3% of clicks**) → $1,252.11 ordered revenue → $37.56 commission (**3.0% effective**) → **$36.06 net**. Net EPC **$0.44/click**, net RPS **$0.188/session**, run rate **$63.64/mo**, risk-adjusted **~$54/mo** after ~$10/mo return drag (15% return rate, 2/13 orders).
- **The funnel is not the problem** — 43% affiliate CTR and 7.3% order conversion are both strong. Constraints are traffic volume and the 3% commission rate.
- **Traffic required:** $100/mo = 1.6x today; $250/mo = 3.9x; $500/mo = 7.9x; **$1,000/mo = 15.7x**. The $1k target is a 2027 conversation, not a content tweak.
- **TOP FINDING — the Steelcase Gesture branded cluster is unclaimed: 37,910 searches/mo across 20 keywords, TCA ranks for none.** `/review/gesture/` is 5,270 words at pos 7.9 with **zero query-attributed impressions** — the only first-hand-authority page is invisible for its own brand terms. Its title targets the tall-fit angle, not the 14,800/mo generic term; only 4 Amazon links vs 15 on the money hub. Conservative capture (pos 5–10, 1.5% of cluster) = 578 visits/mo = 1.7x total current traffic ≈ **+$109/mo**. Recorded on [[review-gesture]].
- **AI referral channel quantified for the first time: 17% of all sessions arrive from ChatGPT/Perplexity/Copilot**, consistently across 19 Clarity windows since mid-June, peaking at 42%. Free, compounding, invisible to GSC.
- **12-month scenarios from $54/mo baseline:** plateau $644 · base (8%/mo) $1,099 · base+Gesture $1,914 · upside $3,093. **Most likely $1,100–1,900, exiting at $135–245/mo.**
- **Levers ranked:** (1) reposition Gesture review onto brand head terms; (2) escape the 3% ceiling — Humanscale/Crandall/FlexiSpot applications **still pending since Jul 4**, Jackson-only, the only lever that multiplies revenue without new traffic; (3) per-page tracking IDs (45% of clicks still "Unknown"); (4) more height-specific pages (`/office-chairs-for-6-foot-6/` converts at 2.76% CTR, **55x** `/knee-pain-seat-depth/`); (5) lean into AI referrals.
- **Confidence guardrail:** whole model rests on 13 orders across 2 periods; GA4 unusable Jun 16–Jul 18 so Clarity (11.3 sessions/day) is the only traffic proxy. August affiliate report + first clean GA4 month are the two data points that would most change this. Wiki updated: [[affiliate-performance]], [[review-gesture]].

## [2026-07-21] audit | Full SEO audit (`/seo audit`) — 76/100. Two critical live trust defects + a stale deploy.

- Ran `/seo audit tallchairadvisor.com`: 6 specialist agents in parallel (technical, content, schema, sitemap, performance, visual) + orchestrator sweep of on-page and GEO. All 43 URLs via curl/urllib per the CLAUDE.md `<head>` rule. Raw: `raw/audits/2026-07-21-full-seo-audit.md`.
- **Score 76/100** — Technical 80 · Content 62 · On-Page 88 · Schema 66 · Performance 88 · Images 78 · AI Readiness 70. Technical execution is strong; the score is dragged down almost entirely by content trust defects that are live in production.
- **C1 — production deploy is stale, GA4 still blocked.** CSP fix `db75ffd` (Jul 18) never shipped; live header matches `9085eca` byte-for-byte. No CD workflow exists — Cloudflare auto-deploys on push and all 4 commits since carry `[skip cd]`. The analytics blackout since ~Jun 16 is **ongoing, not fixed**. New page: [[deploy-pipeline-integrity]].
- **C2 — `/review/leap-plus/` source is destroyed** (raw LLM chat output at line 1, ahead of the frontmatter fence). Astro never runs the frontmatter; **build passes green** but the page emits no title, canonical or JSON-LD. Committed in `3505a12`, unpushed — live is clean, the next deploy ships it. This is the site's #1 click source (34 clicks/90d, 16.3%). Recorded on [[review-leap-plus]].
- **C3 — `/about/` publishes a fabricated testing protocol (LIVE).** "I use every primary review chair as my main seat for at least three weeks" on the same page that discloses hands-on with only the Gesture. Highest-risk page on the site. New page: [[content-integrity]].
- **C4 — cross-page factual contradiction (LIVE).** `/review/gesture/` says 32" inseam / ~3 finger-widths / "not a borderline fit"; `/office-chairs-for-6-foot-4/` says 34" / 1.5–2 finger-widths — including inside FAQPage JSON-LD. `/review/gesture/` also contradicts itself: its Direct Answer capsule still says "borderline fit" while the body says the opposite 4x. The May 2026 fix never propagated.
- **C5 — `aggregateRating` with `reviewCount: 1` on all 7 rating-bearing pages**, each mirroring the single self-authored review. Textbook snippet-spam pattern. Recorded on [[schema-markup]].
- **H1 — 5-page commercial orphan island** (zero inbound links): heavy-duty, monitor-arm, wide-seat, return-policy, best-big-and-tall. **Three are not in GSC at all**; the other two have 21 impressions / 0 clicks combined vs 471/13 for the well-linked `/office-chairs-for-6-foot-6/`. Recorded on [[internal-linking]].
- **H3/H4 — FTC + monetization:** 15 monetized pages have missing or bottom-only disclosure ([[affiliate-compliance]]); `/office-chairs-for-6-foot-6/` — the best-converting format (2.76% CTR) — has ONE Amazon link, at 86.8% scroll depth, as a 17px inline text link.
- **Corrected 2 wrong agent claims:** (1) "remove `*.clarity.ms` from CSP as dead" — false, Clarity loads inline; removing it would kill the only working analytics source. (2) "`Review` schema asserts first-hand testing" — overstated; `reviewBody` text is careful spec analysis, the defect is the `aggregateRating`, not the `Review` node. Both recorded as RULEs on [[schema-markup]].
- **Verified healthy:** 43/43 titles/metas/canonicals/OG/twitter present, all canonicals self-referencing, zero duplicates, zero missing H1s, zero JSON-LD parse errors, zero images missing alt, CLS 0.0 measured across 10 Playwright captures, sitemap 43/43 at 200 with no redirect hops, all AI crawlers explicitly allowed, `llms.txt` present and high quality.
- **GEO gap:** `llms.txt` lists 4 dead URLs — all 301'd in the July consolidation, including `/best-office-chairs/` as the *first* key page. The consolidation updated nav, breadcrumbs, links and sitemap but missed this file. With AI referrals at 17% of sessions, that's the one file those crawlers read.

## [2026-07-21] audit addendum | Performance agent landed — site renders body copy in Times (invalid CSS)

- The 6th audit agent (Core Web Vitals) completed after the main report. Findings appended to `raw/audits/2026-07-21-full-seo-audit.md`; new concept page [[core-web-vitals]].
- **C6 — CRITICAL, verified independently in real Chromium: the entire site renders body copy in Times.** `tailwind.config.mjs:9` declares `sans: ["Source Sans 3", ...]` and Tailwind emits it **unquoted** — `font-family:Source Sans 3,system-ui,sans-serif`. The bare `3` is a number token, not a `<custom-ident>`, so the value is a parse error and Chrome **discards the whole declaration**. Live verification: `getComputedStyle(document.body).fontFamily` → `Times`; the body CSSOM rule has no `font-family` at all; the unquoted form parses to `""` while the quoted form is valid; only 2 of 26 `@font-face` rules load (Playfair 500/600) — Source Sans 3 never downloads. Headings render correctly, which is why it went unnoticed. Fix = quote the family, **but ship with the Playfair preload or it trades a rendering fix for a CLS regression**.
- **Lab CWV all pass:** Lighthouse 97–99, LCP 1,722–1,982 ms, CLS 0.002–0.075, TBT 37–147 ms across 8 pages. INP proxy 88 ms @4x CPU / 104 ms @6x. All 43 URLs brotli, TTFB 50–79 ms, `cf-cache-status: HIT`. **But there is zero field data** — no CrUX key in the repo, anonymous PSI quota exhausted. Getting a key is the biggest performance unblock.
- **Third-party is 63–73% of page weight.** gtag 164.5 KB (64 KiB unused), Clarity 25.2 KB, **Cloudflare Insights 11.6 KB — a third analytics tool**. Removing CF Insights + deferring Clarity returns ~37 KB and 20–36 ms/page.
- **Heroes 3.5–4.5x oversized, zero `srcset` sitewide** (~200 KB/view wasted on the two review pages). **LCP element is TEXT on all 8 pages** — so `preloadImage` is misaimed; `/` preloads a 128×128 avatar ahead of the stylesheet.
- **Two methodology RULEs recorded:** never measure INP in headless Chrome (the agent's first runs showed a constant ~560 ms artifact that didn't scale with throttling — it correctly caught and discarded this); never trust a single Lighthouse run here (±700 ms LCP variance).
- Score unchanged at 76 — the framework has no category that captures a site-wide rendering defect, noted explicitly in the report.

## [2026-07-22] fixes | Critical audit findings C1–C5 remediated + build gate added

Branch `worktree-gsc-post-consolidation-analysis` (PR #1). Two subagents ran the mechanical edits in parallel on non-overlapping file sets; the judgment-heavy reconciliation was done directly. Full verification below.

- **C1 (stale deploy / GA4 blocked) — resolves on merge.** No code change needed: this branch is based on `db75ffd`, so `public/_headers` already carries the GA4 endpoints (`stats.g.doubleclick.net` present, verified). Merging PR #1 is a non-`[skip cd]` commit, which triggers the Cloudflare rebuild that ships the fix. **Verify after deploy:** `curl -sI https://tallchairadvisor.com/ | grep -o "connect-src[^;]*;"` must contain `stats.g.doubleclick.net`.
- **C2 (leap-plus.astro destroyed) — the corruption is NOT in this branch.** `head -1` is `---`; the branch predates `3505a12`. The corruption lives only in Jackson's **unpushed local main** (`27b556d`, `3505a12`) and must be fixed there before those commits are pushed. **Durable fix shipped instead:** `scripts/verify-build.mjs`, wired into `npm run build`. See [[deploy-pipeline-integrity]].
- **C3 (`/about/` fabricated protocol) — FIXED.** "How I Evaluate Chairs" rewritten into two labeled tracks; the "minimum 3 weeks" H3 and the digital-caliper claim are gone; the lede no longer claims physical measurement across all reviews.
- **C4 (cross-page contradiction) — FIXED.** 6'4" page reconciled to 32" inseam / ~3 finger-widths across all 5 sites including the FAQPage JSON-LD; Gesture Direct Answer capsule no longer contradicts its own body; Gesture break-in fabrication removed from `gesture-vs-leap-plus`, `shoulder-pain-tall-people`, `author/jackson-christopher`, and `review/leap-plus` (3 spots incl. FAQ schema). **Also corrected the inverted biomechanics the numbers sat on** — see [[content-integrity]].
- **C5 (`aggregateRating` reviewCount:1) — FIXED on all 7 pages.** All 4 `Review` nodes deliberately preserved. See [[schema-markup]].
...
## [2026-07-23] verify | GA4 affiliate_click link tracking — confirmed working

Verified the GA4 link-tracking pipeline end to end. Live HTML shows GA4 (G-TWK4EPV8DT) + the affiliate_click handler shipping; all money-page links match the tracker condition (www.amazon.com + tag=tallchairadvi-20); a logic simulation confirmed the handler tracks the right links and skips untagged/external ones; transport_type:'beacon' survives navigation. No code bug — nothing to fix in code. Remaining gap is GA4-side config (register 6 custom dimensions, mark affiliate_click a Key event) so params show in reports. Live browser click test not run (extension disconnected); DebugView is the user-side equivalent. Documented in [[affiliate-performance]] "GA4 Link Tracking — Verified Working".
