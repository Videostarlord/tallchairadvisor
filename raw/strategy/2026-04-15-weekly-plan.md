...
- [ ] FIX: /knee-pain-seat-depth/ | Rewrite title tag to ≤60 chars and rewrite meta description to lead with the fix/answer, not the problem — target searchers looking for seat depth solutions, not "knee brace" queries; example title: "Seat Depth & Knee Pain: The Fix for Tall People" | 292 impressions, pos 9.2, 0 clicks; audit identifies title overlength and meta not leading with fix as the primary failures; 292 impressions is actionable signal | FILE: src/pages/knee-pain-seat-depth.astro

---

## NEW CONTENT (Friday agent)

- [ ] NEW: Height-Bracket Verdict Table on /best-office-chairs/ | "best office chairs for tall people 6'3 6'4 6'5 6'6" | Addition to existing /best-office-chairs/ page — insert a structured Height-Bracket Verdict Table in the format: "At 6'X → min seat height ≥X", min seat depth ≥X" → Passing chairs: [list with Amazon affiliate links tag=tallchairadvi-20]" covering 6'0"–6'7" brackets; this is flagged as the highest AI Overview citation probability on the site and is a content addition to an existing money page, not a new page — the page itself is in the recently-edited list but this is a structural content block addition, treat as append-only; score against /blog-analyze before publishing

> **Note to Friday agent:** /best-office-chairs/ is in the recently-edited list. The Height-Bracket Verdict Table is a net-new structural section (append, not rewrite) targeting AI Overview citations. It does not alter existing content. If the agent's interpretation of cooldown rules prohibits this, defer to next week and flag for human review.
...
*No rewrites scheduled this week.* All pages with actionable impression signals are either receiving targeted meta fixes above (technical-level changes) or are in the recently-edited cooldown window. Per cadence rules, rewrites are not warranted where a meta fix is the diagnosed failure mode.
